...
REM
REM Environment setup file for EasyRuby.
REM
...
REM
REM Now Determine the EasyRuby Root directory (parent directory of Ruby)
REM
PUSHD %RUBY_DIR%\..
SET ROOT_DIR=%CD%
POPD
...
REM
REM Add RUBY_DIR\bin to the PATH and then Git\cmd
REM RUBY_DIR\bin takes higher priority to avoid other tools conflict
REM
SET PATH=%RUBY_DIR%\bin;%RUBY_DIR%\lib\ruby\gems\1.9.1\bin;%ROOT_DIR%\Git\cmd;%PATH%
SET RUBY_DIR=
SET ROOT_DIR=
...
IF EXIST %EasyRubyPath% (
  ruby %EasyRubyPath%\scripts\config_check.rb
) ELSE (
